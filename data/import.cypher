...
LOAD CSV WITH HEADERS FROM 'file:///vagrant/data/liquids.csv' AS line
CREATE (:Ingredient { index: line.index, name: line.name});

CREATE CONSTRAINT ON (re:Recipe) ASSERT re.index IS UNIQUE;
USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM 'file:///vagrant/data/recipes.csv' AS line
CREATE (:Recipe { index: line.index, name: line.name});

CREATE CONSTRAINT ON (re:Recipient) ASSERT re.index IS UNIQUE;
USING PERIODIC COMMIT
LOAD CSV WITH HEADERS FROM 'file:///vagrant/data/recipient.csv' AS line
CREATE (:Recipient { index: line.index, name: line.name});

LOAD CSV WITH HEADERS FROM 'file:///vagrant/data/recipes_link_liquids.csv' AS line
...
LOAD CSV WITH HEADERS FROM 'file:///vagrant/data/recipes_link_recipient.csv' AS line
MATCH (re:Recipe) WHERE re.index=line.id_recipe
